...
import { ArrowRight, AlertTriangle, Shield, Globe, Lightbulb, LightbulbOff } from 'lucide-react';
...
void main() {
    ...
    // Define base colors from a dark, sophisticated palette
    vec3 color1 = vec3(0.02, 0.03, 0.05); // Deepest slate
    vec3 color2 = vec3(0.05, 0.07, 0.12); // Deep indigo
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background: 'radial-gradient(circle at 50% 0%, rgba(255, 180, 50, 0.15) 0%, transparent 60%)',
      ...
      <div className="relative flex flex-col items-center z-50 shrink-0">
        ...
        >
          ...
          {/* Interactive Bulb */}
          <div
            className={`w-8 h-8 rounded-full border border-white/10 -mt-1 flex items-center justify-center transition-all duration-700 ${isLightOn ? 'bg-[#fff6e5] scale-110 shadow-[0_0_40px_rgba(255,200,100,0.8),0_0_100px_rgba(255,200,100,0.4)]' : 'bg-[#111] shadow-[0_0_10px_rgba(255,200,100,0.1)] hover:scale-105'
              }`}
          >
            {isLightOn ? (
              <Lightbulb className="w-4 h-4 text-[#ffcc00] opacity-100" />
            ) : (
              <LightbulbOff className="w-4 h-4 text-orange-200/20 group-hover:text-orange-200/40" />
            )}
          </div>
            ...
            >
              ...
              <div className="space-y-4">
                <button
                  ...
                  disabled={isLoading}
                  className="w-full bg-white text-[#07006c] h-14 rounded-full flex items-center justify-center gap-3 font-semibold group transition-all duration-300 hover:bg-opacity-90 active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isLoading ? (
                    <div className="w-5 h-5 border-2 border-[#07006c]/30 border-t-[#07006c] rounded-full animate-spin" />
                  ) : (
                    <>
              ...
              <footer className="text-center flex flex-col items-center">
                <a className="text-[#c6c6cd] text-sm hover:text-[#bec6e0] transition-colors duration-300 group inline-flex items-center gap-1 mb-6" href="/register">
                  Don't have an account? <span className="text-[#bec6e0] font-semibold underline underline-offset-4 decoration-[#bec6e0]/30 group-hover:decoration-[#bec6e0]">Sign up</span>
                </a>
