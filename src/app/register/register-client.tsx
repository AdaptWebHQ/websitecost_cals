...
/* ─────────────────────────────────────────────
   DESKTOP — Cogie split-screen (Register variant)
───────────────────────────────────────────── */
function DesktopRegister({ isLoading, authError, error, onGoogle }: { isLoading: boolean; authError: string | null; error: string | null; onGoogle: () => void }) {
  return (
    <div style={{ fontFamily: "'Inter',-apple-system,sans-serif", background: '#000', display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', padding: '40px 20px' }}>
      <style>{`@keyframes _rspin{to{transform:rotate(360deg)}}`}</style>
      <div style={{ width: '100%', maxWidth: 1500, height: 'min(88vh,1000px)', borderRadius: 28, overflow: 'hidden', display: 'grid', gridTemplateColumns: '1fr 1fr', boxShadow: '0 40px 100px rgba(0,0,0,0.55),0 0 0 1px rgba(255,255,255,0.06)', background: '#fff' }}>
        ...
        {/* Left art */}
        <div style={{ position: 'relative', background: '#050203', overflow: 'hidden', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: '48px 52px' }}>
          <FlowCanvas />
          ...
          <div style={{ position: 'relative', zIndex: 2 }}>
            <h1 style={{ fontFamily: "'Playfair Display',serif", fontWeight: 600, fontSize: 'clamp(38px,4.4vw,58px)', lineHeight: 1.04, letterSpacing: '-0.01em', marginBottom: 20, color: '#fff', textShadow: '0 2px 24px rgba(0,0,0,.35)' }}>
              Start Your<br />Journey Today
            </h1>
            <p style={{ fontSize: 15, lineHeight: 1.6, color: 'rgba(255,255,255,.78)', maxWidth: 380, fontWeight: 400 }}>
              The secret of getting ahead is getting started. Create your account in seconds.
            </p>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '56px 64px 40px', overflowY: 'auto', background: '#fff' }}>
          ...
          <div style={{ flex: 1, width: '100%', maxWidth: 400, display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '32px 0' }}>
            <div style={{ textAlign: 'center', marginBottom: 34 }}>
              ...
              <h2 style={{ fontFamily: "'Playfair Display',serif", fontWeight: 600, fontSize: 40, letterSpacing: '-0.01em', color: '#0b0b0d', marginBottom: 10 }}>Create Account</h2>
              <p style={{ fontSize: 14.5, color: '#6b6b70', fontWeight: 400, maxWidth: 300, margin: '0 auto', lineHeight: 1.5 }}>
                Sign up with Google to get started — it only takes a second
              </p>
            </div>
            ...
            >
              {isLoading ? <div style={{ width: 20, height: 20, border: '2px solid #ccc', borderTopColor: '#555', borderRadius: '50%', animation: '_rspin .7s linear infinite' }} /> : <><GoogleIcon />Sign Up with Google</>}
            </button>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, marginTop: 22, fontSize: 12.5, color: '#6b6b70' }}>
              ...
              Secure sign-up, no password required
            </div>

            {/* Terms */}
            <p style={{ textAlign: 'center', fontSize: 12, color: '#9a9a9d', marginTop: 20, lineHeight: 1.6 }}>
              By signing up, you agree to our{' '}
              <a href="/terms" style={{ color: '#0b0b0d', fontWeight: 500, textDecoration: 'none' }}>Terms of Service</a>{' '}
              and{' '}
              <a href="/privacy" style={{ color: '#0b0b0d', fontWeight: 500, textDecoration: 'none' }}>Privacy Policy</a>.
            </p>
          </div>

          <div style={{ textAlign: 'center', fontSize: 13.5, color: '#6b6b70', marginTop: 36 }}>
            Already have an account?{' '}
            <a href="/login" style={{ color: '#0b0b0d', fontWeight: 600, textDecoration: 'none' }}>Sign in</a>
          </div>
        </div>
      </div>
    </div>
...
/* ─────────────────────────────────────────────
   MOBILE — original lamp card (Register)
───────────────────────────────────────────── */
function MobileRegister({ isLoading, authError, error, onGoogle }: { isLoading: boolean; authError: string | null; error: string | null; onGoogle: () => void }) {
  ...
  useEffect(() => {
    ...
    const fs = `precision highp float;uniform float u_time;uniform vec2 u_resolution;varying vec2 v_texCoord;void main(){vec2 uv=v_texCoord;float noise=sin(uv.x*10.+u_time*.5)*cos(uv.y*10.+u_time*.3)*.5+.5;vec3 c1=vec3(.02,.015,.04);vec3 c2=vec3(.06,.04,.1);vec3 c=mix(c1,c2,noise*.2+uv.y*.1);float v=1.-length(uv-.5)*1.2;c*=smoothstep(0.,1.,v);gl_FragColor=vec4(c,1.);}`;
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[150vw] h-[150vh] pointer-events-none transition-opacity duration-1000" style={{ background: 'radial-gradient(circle at 50% 0%,rgba(139,92,246,.15) 0%,transparent 60%)', opacity: isLightOn ? 1 : 0 }} />
      ...
      <div className="relative flex flex-col items-center z-50 shrink-0">
        ...
        <button onClick={() => setIsLightOn(!isLightOn)} className="group relative flex flex-col items-center cursor-pointer focus:outline-none">
          ...
          <div className={`w-8 h-8 rounded-full border border-white/10 -mt-1 flex items-center justify-center transition-all duration-700 ${isLightOn ? 'bg-[#f0eaff] scale-110 shadow-[0_0_40px_rgba(139,92,246,.8),0_0_100px_rgba(139,92,246,.4)]' : 'bg-[#111] hover:scale-105'}`}>
            {isLightOn ? <Lightbulb className="w-4 h-4 text-[#a78bfa]" /> : <LightbulbOff className="w-4 h-4 text-purple-200/20 group-hover:text-purple-200/40" />}
          </div>
        ...
        <AnimatePresence>
          {isLightOn && (
            <motion.div initial={{ opacity: 0, y: 20, scale: .95 }} animate={{ opacity: 1, y: 0, scale: 1, rotateX, rotateY }} exit={{ opacity: 0, y: 20, scale: .95 }} transition={{ type: 'spring', stiffness: 300, damping: 30, rotateX: { type: 'tween', ease: 'linear', duration: .1 }, rotateY: { type: 'tween', ease: 'linear', duration: .1 } }}
              ...
              <button onClick={onGoogle} disabled={isLoading} className="w-full bg-white text-[#2e1065] h-14 rounded-full flex items-center justify-center gap-3 font-semibold group transition-all duration-300 hover:bg-opacity-90 active:scale-[.98] disabled:opacity-50 disabled:cursor-not-allowed">
                {isLoading ? <div className="w-5 h-5 border-2 border-[#2e1065]/30 border-t-[#2e1065] rounded-full animate-spin" /> : <><GoogleIcon /><span>Sign Up with Google</span><ArrowRight className="w-5 h-5 transition-transform duration-300 group-hover:translate-x-1" /></>}
              </button>
              <footer className="text-center flex flex-col items-center">
                <a className="text-[#c6c6cd] text-sm hover:text-[#bec6e0] transition-colors duration-300 group inline-flex items-center gap-1 mb-6" href="/login">
                  Already have an account? <span className="text-[#bec6e0] font-semibold underline underline-offset-4">Sign in</span>
                </a>
                <div className="text-center text-[10px] text-[#c6c6cd]/60 leading-relaxed max-w-[280px] mb-6">
                  By signing up, you agree to our <a href="/terms" className="hover:text-[#c6c6cd]">Terms of Service</a> and <a href="/privacy" className="hover:text-[#c6c6cd]">Privacy Policy</a>.
                </div>
...
function RegisterFormContent() {
  ...
  return isDesktop ? <DesktopRegister {...props} /> : <MobileRegister {...props} />;
}
