...
export async function sendInquiryEmail(inquiry: {
  ...
  try {
    ...
    const emailSubject = `New CRM Lead Inquiry: ${escapeHtml(inquiry.companyName || 'N/A')} (${escapeHtml(inquiry.name || 'N/A')})`;
    const emailHtml = `
      <h2>New Calculator Lead Received</h2>
      <p><strong>Name:</strong> ${escapeHtml(inquiry.name || 'N/A')}</p>
      <p><strong>Company:</strong> ${escapeHtml(inquiry.companyName || 'N/A')}</p>
      <p><strong>Email:</strong> ${escapeHtml(inquiry.email || 'N/A')}</p>
      ...
      <blockquote style="background:#f1f5f9;padding:10px 15px;border-left:4px solid #6366f1;">
        ${escapeHtml(inquiry.message || 'N/A')}
      </blockquote>
      ${calcUrl ? `<p><strong>Linked Calculation ID:</strong> <a href="${calcUrl}">${escapeHtml(inquiry.calculationId!)}</a></p>` : ''}
      <p><a href="${inquiryUrl}">Open Lead in CRM Portal</a></p>
    `;
...
export async function sendWelcomeEmail(user: { name: string; email: string }) {
  ...
  const welcomeSubject = `Welcome to AdaptWeb Cost Calculator, ${escapeHtml(user.name)}!`;
  const welcomeHtml = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 25px; border: 1px solid #e2e8f0; border-radius: 8px;">
      ...
      <p style="text-align: center; margin: 30px 0;">
        <a href="${appUrl}/public/calculator" style="background-color: #4f46e5; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; font-weight: bold;">Launch Cost Calculator</a>
      </p>
      <hr style="border: 0; border-top: 1px solid #e2e8f0; margin: 30px 0;" />
      <p style="font-size: 12px; color: #64748b; line-height: 1.4;">
        This email was sent to ${escapeHtml(user.email)} as a registration confirmation. If you did not sign up for AdaptWeb Cost Calculator, please ignore this email.
      </p>
    </div>
  `;
  return await sendMail({
...
export async function sendSecurityAlertEmail(user: { name: string; email: string }, loginTime: string) {
  // Fixed: was incorrectly using webcostpro.com as fallback domain
  const appUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://adaptweb.in';
  ...
  const loginHtml = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 25px; border: 1px solid #e2e8f0; border-radius: 8px;">
      ...
      <p>Hi <strong>${escapeHtml(user.name)}</strong>,</p>
      <p>Your AdaptWeb Cost Calculator account was recently accessed on <strong>${escapeHtml(loginTime)}</strong>.</p>
      
      <table style="width: 100%; border-collapse: collapse; margin: 20px 0; font-size: 14px;">
        <tr>
          <td style="padding: 8px 0; color: #64748b; width: 120px;"><strong>Account:</strong></td>
          <td style="padding: 8px 0; color: #0f172a;">${escapeHtml(user.email)}</td>
        </tr>
        <tr>
          <td style="padding: 8px 0; color: #64748b;"><strong>Time:</strong></td>
          <td style="padding: 8px 0; color: #0f172a;">${escapeHtml(loginTime)}</td>
        </tr>
      </table>

      <p>If this was you, no action is needed. You're ready to continue tracking and creating website cost estimates!</p>
      <p style="margin-top: 20px;">
        <a href="${appUrl}/public" style="color: #4f46e5; font-weight: bold; text-decoration: underline;">Go to Dashboard</a>
      </p>
      <hr style="border: 0; border-top: 1px solid #e2e8f0; margin: 30px 0;" />
...
export async function sendLeadAssignmentEmail(
  ...
  try {
    ...
    const emailSubject = `🚨 New Lead Assigned to You: ${escapeHtml(inquiry.companyName || 'N/A')} (${escapeHtml(inquiry.name || 'N/A')})`;
    const emailHtml = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 25px; border: 1px solid #e2e8f0; border-radius: 8px;">
        <h2 style="color: #4f46e5; margin-top: 0; margin-bottom: 20px;">Lead Assigned Notification</h2>
        <p>Hi <strong>${escapeHtml(assignee.name)}</strong>,</p>
        <p>You have been assigned a new lead by <strong>${escapeHtml(assignedBy)}</strong>.</p>
        ...
        <p style="text-align: center; margin: 30px 0;">
          <a href="${inquiryUrl}" style="background-color: #4f46e5; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; font-weight: bold; display: inline-block;">Open Lead in CRM Portal</a>
        </p>
        <hr style="border: 0; border-top: 1px solid #e2e8f0; margin: 30px 0;" />
        <p style="font-size: 11px; color: #94a3b8; line-height: 1.4;">
